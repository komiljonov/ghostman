...
func TestHeadersAndQueryParamsValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{"enabled not a bool", `[{"key":"k","value":"v","enabled":"yes"}]`, "FIELD[0].enabled must be true or false"},
		{"enabled null", `[{"key":"k","value":"v","enabled":null}]`, "FIELD[0].enabled must be true or false"},
